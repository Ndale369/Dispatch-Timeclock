...
async function saveUsers(updatedUsers) {
  ...
  // ⬇️ Instead of kv.set, call your save function
  saveUsers(updated);
...
useEffect(() => {
  ...
  if (stored) setRounding(stored);
}, []);
  const [autoClockOut, setAutoClockOut] = useState("none");

useEffect(() => {
  const stored = localStorage.getItem("autoClockOut");
  if (stored) setAutoClockOut(stored);
}, []);
  ...
  const DashboardPage = () => {
    const user = currentUserId && !isAdmin ? getUserById(currentUserId) : null;
  ...
  const currentUser =
    currentUserId && !isAdmin ? getUserById(currentUserId) : null;
